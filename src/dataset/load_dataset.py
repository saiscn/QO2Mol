...
def prepare_train_dataset(root_dir, split_valid=True, random_split=False):
    tr_dataset = QO2MolDataset(root=root_dir)
    split_indices = tr_dataset.get_split_indices()
    if random_split:
        num_samples = len(tr_dataset)
        rand_indices = np.random.shuffle(list(range(num_samples)))
    ...
    if split_valid:
        perm = torch.randperm(len(train_split))
        train_num = int(0.8 * len(train_split))
